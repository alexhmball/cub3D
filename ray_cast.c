...
/*   Updated: 2023/02/07 11:23:08 by aball            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_texture	*find_texture(t_data *data, double *distv, double *disth, int *w_height)
{
	t_texture *texture;
...
void draw_wall_slice(t_data *data, int x)
{
	...
	if (start < 0)
	{
		texture->y = -(double)start / (double)w_height;
		start = 0;
	}
	while (start < end)
	{
