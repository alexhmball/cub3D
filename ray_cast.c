...
/*   Updated: 2023/02/03 17:30:30 by aball            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cast_horizonal(t_data *data)
{
	...
	if (quad == 3 || quad == 4)
	{
		...
		data->player.ray.hx_inc = 64 / tan(degtorad(data->player.ray.angle));
	}
	else
	{
		data->player.ray.hy_inc = 64;
		data->player.ray.hy = floor(data->player.ray.y) + 65;
		data->player.ray.hx = (data->player.ray.y - data->player.ray.hy) / tan(degtorad(data->player.ray.angle)) + data->player.ray.x;
		data->player.ray.hx_inc = 64 / tan(degtorad(data->player.ray.angle));
	}
}
...
void	cast_vertical(t_data *data)
{
	...
	if (quad == 3 || quad == 2)
	{
		...
		data->player.ray.vy_inc = 64 * tan(degtorad(data->player.ray.angle));
	}
	else
	{
		data->player.ray.vx_inc = 64;
		data->player.ray.vx = floor(data->player.ray.x) + 65;
		data->player.ray.vy = ((data->player.ray.x - data->player.ray.vx) * tan(degtorad(data->player.ray.angle))) + data->player.ray.y;
		data->player.ray.vy_inc = 64 * tan(degtorad(data->player.ray.angle));
	}
...
void draw_wall_slice(t_data *data, int x)
{
	...
	distv = distance(data->player.ray.vx, data->player.ray.vy, data->player.x_pos * 64, data->player.y_pos * 64);
	disth = distance(data->player.ray.hx, data->player.ray.hy, data->player.x_pos * 64, data->player.y_pos * 64);
	// blue = 255 - dist / 5;
	// red = 255 - dist / 5;
	colour = 0xff00ff;
	// if (distv < disth)
	// 	w_height = (64 / distv) * 512;
	// else
		w_height = (64 / disth) * 512;
...
{
	if (floor(data->player.ray.vy / 64) < 0 || floor(data->player.ray.vx / 64) < 0)
		return (0);
	if (floor(data->player.ray.hy / 64) < 0 || floor(data->player.ray.hx / 64) < 0)
		return (0);
	if (data->map.map[(int)floor(data->player.ray.hy / 64)][(int)floor(data->player.ray.hx / 64)] == '1')
		return (0);
	if (data->map.map[(int)floor(data->player.ray.vy / 64)][(int)floor(data->player.ray.vx / 64)] == '1')
		return (0);
...
void	cast_ray(t_data *data)
{
	...
	data->player.ray.x = data->player.x_pos * 64;
	data->player.ray.y = data->player.y_pos * 64;
	rays = 0;
	while (rays < FOV * 20)
	{
		cast_horizonal(data);
		...
		printf("horizonal x: %f y: %f\n", data->player.ray.hx / 64, data->player.ray.hy / 64);
			...
			// my_mlx_pixel_put(data, data->player.ray.hx, data->player.ray.hy, 0xFFFFFF);
			// my_mlx_pixel_put(data, data->player.ray.vx, data->player.ray.vy, 0x000000);
		}
